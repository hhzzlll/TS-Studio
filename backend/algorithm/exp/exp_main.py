...
from backend.algorithm.exp.exp_basic import Exp_Basic
from backend.algorithm.models_diffusion import DDPM

from backend.algorithm.utils.metrics import metric, calc_quantile_CRPS
from backend.algorithm.utils.tools import visual, visual_prob, visual2D
from backend.algorithm.FEA_module.FEA import *
from backend.algorithm.FEA_module.mask import *

class Exp_Main(Exp_Basic):
    def __init__(self, args, callback=None):
        super(Exp_Main, self).__init__(args)
        self.callback = callback
        # System Integration: Resolve config path dynamically
        config_path = 'config/config_FEA.json'
        
        # Try to find config relative to this file if not found locally
        if not os.path.exists(config_path):
             current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
             candidate = os.path.join(current_dir, 'config', 'config_FEA.json')
    ...
    def pretrain(self, setting):
        ...
        for epoch in range(self.args.pretrain_epochs):
            ...
            if self.callback:
                self.callback('pretrain', epoch + 1, self.args.pretrain_epochs, None)
